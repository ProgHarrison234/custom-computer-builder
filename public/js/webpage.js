    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < componentName.length; j++)
        if (data[i].component === componentName[j]) {
          $(`[id = '${componentName[j]}'`)
            .append(`<a class="dropdown-item" title = "${data[i].description}">
...
$("#save-for-later").on("click", function() {
  ...
  //let [cpu, cooler, motherboard, ram, storage, card, psu, compcase] = array;
  let array = Array.from(document.querySelectorAll(".data")).map(x =>
    x.getAttribute("data-id")
  );
  let queryUrl = "/api/create-build/" + array.join("/");
